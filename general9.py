...
P1 = alpha - s*q2 - s*q3 - q1
P2 = alpha - s*q1 - q2
P3 = alpha - s*q1 - q3

C1 = cost - a1 - a2 - n*b1 - n*b2
...
Utility= alpha*(soln[q1]) - ((soln[q1])**2)/2 - s*((soln[q1])*(soln[q2])) - s*((soln[q1])*(soln[q3])) - ((p1)*(soln[q1])) + alpha*(soln[q2]) - ((soln[q2])**2)/2 - s*((soln[q2])*(soln[q1])) - ((p2)*(soln[q2])) + alpha*(soln[q3]) - ((soln[q3])**2)/2 - s*((soln[q3])*(soln[q1])) - ((p3)*(soln[q3]))
...
plt.title('Network E | b=0.5')
plt.xlabel('γ')
plt.ylabel('Welfare')
plt.text(0.47, f_max_val -5, f"Maximum Welfare: {f_max_val:.2f} at γ={n_max:.2f}", ha='center')
plt.show()